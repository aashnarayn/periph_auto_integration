...
cd ../my_soc/hw && make BOARD=nexys_video quick_build_automated
...
| `status` | (E) report whether a clone exists, is injected, and which config it holds. |
...
The targets `setup_full`, `setup_base`, `remove_automation`, `status`, `delete_clone`, and `update_framework` function identically to the V2 manager, but interact exclusively with the V1 engine payload.